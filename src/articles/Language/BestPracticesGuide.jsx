...
/* ---------- Practice Tracker: weekly habit grid + streak, persisted ---------- */
const HABITS = [
  { id: "retrieval", label: "Truy xuất (SRS/flashcard)", hint: "che đáp án, tự nhớ lại", tone: LAYER.mech.tone },
  { id: "input", label: "Input dễ hiểu", hint: "đọc/nghe ~95% hiểu", tone: LAYER.bp.tone },
  { id: "output", label: "Output (nói/viết)", hint: "self-talk / nhật ký / hội thoại", tone: LAYER.small.tone },
  { id: "sound", label: "Phát âm / thanh điệu", hint: "shadowing / cặp tối thiểu", tone: LAYER.pitfall.tone },
];
...
function PracticeTracker({ section }) {
  const [grid, setGrid] = useState({});
  const [lang, setLang] = useState("en");
  const [goal, setGoal] = useState("");
  const [loaded, setLoaded] = useState(false);
  const wk = useMemo(() => weekKey(), []);
  const storeKey = "practice:" + wk;
  ...
  const persist = async (next) => {
    try { await window.storage.set(storeKey, JSON.stringify(next)); } catch (e) {}
  };

  const toggle = (habit, day) => {
    const k = habit + ":" + day;
    const next = { ...grid, [k]: !grid[k] };
    setGrid(next);
    persist({ grid: next, lang, goal });
  };

  const cellsDone = Object.values(grid).filter(Boolean).length;
  const total = HABITS.length * DAYS.length;
  const pct = Math.round((cellsDone / total) * 100);

  // streak = consecutive days (from today backwards) with >=1 habit done
  const streak = useMemo(() => {
    let st = 0;
    const todayIdx = (new Date().getDay() + 6) % 7;
    for (let i = todayIdx; i >= 0; i--) {
      const any = HABITS.some((h) => grid[h.id + ":" + i]);
      if (any) st++; else break;
    }
    return st;
  }, [grid]);
  ...
  return (
    <div className="tracker">
      <div className="tk-lead">{section.groups[0].items[0].text}</div>

      <div className="tk-goalrow">
        <label className="tk-goal-label">Mục tiêu tuần này</label>
        <input
          className="tk-goal"
      ...
      <div className="tk-stats">
        <div className="tk-stat">
          <div className="tk-stat-n" style={{ color: LAYER.small.tone }}>{streak}</div>
          <div className="tk-stat-l">chuỗi ngày liên tiếp</div>
        </div>
        <div className="tk-stat">
          <div className="tk-stat-n" style={{ color: LAYER.mech.tone }}>{cellsDone}</div>
          <div className="tk-stat-l">ô đã hoàn thành</div>
        </div>
        <div className="tk-stat">
          <div className="tk-stat-n" style={{ color: LAYER.action.tone }}>{pct}%</div>
          <div className="tk-stat-l">tuần này</div>
        </div>
      </div>

      <div className="tk-grid" role="table">
        <div className="tk-row tk-head" role="row">
          <div className="tk-habit" />
          {DAYS.map((d, i) => (
            <div className="tk-day-h" key={i} role="columnheader">{d}</div>
          ))}
        </div>
        {HABITS.map((h) => (
          <div className="tk-row" key={h.id} role="row">
            <div className="tk-habit">
              <span className="tk-dot" style={{ background: h.tone }} />
              <span className="tk-habit-t">{h.label}</span>
              <span className="tk-habit-h">{h.hint}</span>
            </div>
            {DAYS.map((d, i) => {
              const on = !!grid[h.id + ":" + i];
              return (
                <button
                  key={i}
      ...
      <div className="tk-manifesto">
        <div className="tk-manifesto-h">Lý thuyết → Thực hành → Năng lực</div>
        <p>Bảy phần đầu của tài liệu là bản đồ. Bảng này là bước chân. Không ai giỏi ngoại ngữ vì đọc về cách học — chỉ vì <b>làm, mỗi ngày, đủ lâu</b>. Một ô tick khiêm tốn hôm nay đáng giá hơn một kế hoạch hoàn hảo cho ngày mai.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="root">
      ...
      <aside className="rail">
        <div className="brand">
          ...
          <div>
            ...
            <div className="brand-s">Học ngoại ngữ theo bằng chứng</div>
          </div>
        </div>
        <nav>
          {SECTIONS.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                {s.groups.some((g) => g.isBP) && <span className="nav-tag">{s.groups.filter(g=>g.isBP).length}</span>}
              </button>
            );
          })}
        </nav>
        <div className="rail-foot">Nguồn: Krashen · Long · Swain · Nation · Bjork · Roediger &amp; Karpicke</div>
      </aside>
      ...
      <main className="stage" ref={scrollRef}>
        ...
        {section.special === "tracker" ? (
          <PracticeTracker section={section} />
        ) : active === bpSectionIdx ? (
...
function navLabel(title, i) {
  const NAV = {
    "Tóm tắt điều hành": { num: "00", label: "Tóm tắt điều hành" },
    "Nền tảng khoa học": { num: "01", label: "Nền tảng khoa học" },
    "Chuyển di ngôn ngữ mẹ đẻ": { num: "02", label: "Chuyển di L1" },
    "Nguyên tắc thực hành": { num: "03", label: "Nguyên tắc thực hành" },
    "Lộ trình theo ngôn ngữ": { num: "04", label: "Lộ trình ngôn ngữ" },
    "Hệ thống thực hành": { num: "05", label: "Hệ thống thực hành" },
    "Bảng theo dõi thực hành": { num: "06", label: "Bảng theo dõi" },
    "Nguồn tham khảo": { num: "★", label: "Nguồn tham khảo" },
  };
...
function eyebrow(title) {
  const EB = {
    ...
    "Bảng theo dõi thực hành": "CÔNG CỤ",
    "Nguồn tham khảo": "PHỤ LỤC",
  };
  return EB[title] || "";
}
...
const CSS = `
@import url('https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,500;9..144,600;9..144,700&family=Spline+Sans:wght@400;500;600&family=Spline+Sans+Mono:wght@500;600&display=swap');
...
.brand-mark {
  ...
  border: 1.5px solid #C9762E; color: #E9B778;
...
.nav-item {
  ...
  padding: 10px 20px; font-size: 13.5px; position: relative;
  transition: background .15s, color .15s;
}
.nav-item:hover { background: rgba(255,255,255,0.05); color: #EDE8DB; }
.nav-item.on { background: rgba(233,183,120,0.10); color: #F5EFE1; }
.nav-item.on::before { content:""; position:absolute; left:0; top:0; bottom:0; width:3px; background:#C9762E; }
.nav-num {
  font-family: 'Spline Sans Mono', monospace; font-size: 11px; letter-spacing: 0.06em;
  color: #E9B778; width: 20px; flex-shrink: 0;
}
.nav-label { flex: 1; }
.nav-tag {
  font-family: 'Spline Sans Mono', monospace; font-size: 10px;
  background: #C9762E; color: #1a1208; border-radius: 9px; padding: 1px 7px; font-weight: 600;
}
.rail-foot {
  font-size: 10.5px; color: #7C7767; line-height: 1.5;
...
.stage-title {
  font-family: 'Fraunces', serif; font-weight: 600; font-size: 33px; line-height: 1.12;
  margin: 0; letter-spacing: -0.01em;
}

/* BP list */
.bp-list { max-width: 820px; }
.bp-lead {
...
.bp-card.open { border-color: #C9BBA0; box-shadow: 0 8px 30px -18px rgba(20,20,10,0.35); }
.bp-head {
...
.bp-title { flex: 1; font-family: 'Fraunces', serif; font-size: 20px; font-weight: 600; line-height: 1.25; }
.bp-chev { font-family: 'Spline Sans Mono', monospace; font-size: 20px; color: var(--muted); flex-shrink: 0; }
...
.blk-h2 { font-family: 'Fraunces', serif; font-weight: 600; font-size: 18px; margin: 20px 0 8px; }
.blk-h3 { font-family: 'Spline Sans', sans-serif; font-weight: 600; font-size: 14px; color: var(--accent); margin: 16px 0 6px; letter-spacing: 0.02em; }
...
/* ---- practice tracker ---- */
.tracker { max-width: 820px; }
.tk-lead {
  font-size: 14px; color: var(--muted); line-height: 1.62;
  border-left: 3px solid #4E7A3E; background: #EEF2E9;
  padding: 12px 16px; border-radius: 0 4px 4px 0; margin-bottom: 22px;
}
.tk-goalrow { display: flex; flex-direction: column; gap: 6px; margin-bottom: 20px; }
.tk-goal-label {
...
.tk-goal {
  ...
  font-family: 'Spline Sans', sans-serif; font-size: 14px; background: var(--panel); color: var(--ink);
}
.tk-goal:focus { outline: 2px solid var(--accent); outline-offset: 1px; }
.tk-stats { display: flex; gap: 14px; margin-bottom: 22px; }
.tk-stat {
  flex: 1; background: var(--panel); border: 1px solid var(--rule); border-radius: 6px;
  padding: 14px 16px; text-align: center;
}
.tk-stat-n { font-family: 'Fraunces', serif; font-weight: 600; font-size: 30px; line-height: 1; }
.tk-stat-l { font-size: 12px; color: var(--muted); margin-top: 5px; }
.tk-grid { border: 1px solid var(--rule); border-radius: 6px; overflow: hidden; background: var(--panel); }
.tk-row { display: grid; grid-template-columns: 1fr repeat(7, 42px); align-items: stretch; }
.tk-row + .tk-row { border-top: 1px solid var(--rule); }
.tk-head { background: #EFEDE6; }
.tk-day-h {
  display: flex; align-items: center; justify-content: center;
  font-family: 'Spline Sans Mono', monospace; font-size: 11.5px; font-weight: 600; color: var(--muted);
  border-left: 1px solid var(--rule);
}
.tk-habit { padding: 11px 14px; display: flex; flex-direction: column; gap: 1px; justify-content: center; }
.tk-head .tk-habit { padding: 0; }
.tk-dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; margin-right: 7px; vertical-align: middle; }
.tk-habit-t { font-size: 13.5px; font-weight: 600; color: var(--ink); }
.tk-habit-h { font-size: 11px; color: var(--muted); margin-left: 15px; }
.tk-cell {
  border: none; border-left: 1px solid var(--rule); background: #fff; cursor: pointer;
  font-size: 15px; color: #fff; display: flex; align-items: center; justify-content: center;
  transition: background .12s; font-weight: 700;
}
.tk-cell:hover { background: #F0EEE7; }
.tk-cell.on:hover { filter: brightness(1.08); }
.tk-foot { font-size: 12px; color: var(--muted); margin-top: 12px; font-style: italic; }
.tk-manifesto {
  margin-top: 28px; border: 1px solid #C9BBA0; border-radius: 6px; padding: 18px 20px;
  background: linear-gradient(180deg,#FBFAF6, #F4F1E8);
}
.tk-manifesto-h {
  font-family: 'Spline Sans Mono', monospace; font-size: 12px; letter-spacing: 0.14em;
  text-transform: uppercase; color: var(--accent); margin-bottom: 8px;
}
.tk-manifesto p { margin: 0; font-size: 14.5px; line-height: 1.62; color: #2A2E36; }

@media (max-width: 780px) {
  .tk-row { grid-template-columns: 1fr repeat(7, 34px); }
  .tk-habit-h { display: none; }
